...
void btree_insertCell(uint8_t pb[bt1_pagesize], uint8_t ret[bt1_pagesize], uint8_t ic[bt1_cellsize], int insIdx){
    ...
    for(int i = 0; i < 4; i++){
        ret[btbuffer_celloffset(bt1_cellsperpage)+i] = pb[btbuffer_celloffset(bt1_cellsperpage-1)+bt1_cellpointeroffset+i];
    }
}
...
int btree_findPointerCell(uint8_t pb[bt1_pagesize], uint32_t pIdx){
    ...
    // Don't forget about the end pointer you dunce
    bool isSame = true;
    for(int i = 0; i < 4; i++){
        isSame = isSame && pb[btbuffer_celloffset(bt1_cellsperpage)+i] == pt[i];
    }
    if(isSame){
        return bt1_cellsperpage;
    }
    return -1;
}
